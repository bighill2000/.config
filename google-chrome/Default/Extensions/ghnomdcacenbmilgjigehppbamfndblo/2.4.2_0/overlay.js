...
var camelizer = {
  ...
  constructChartURL: function() {
    ...
    if (context.isBestbuy()) {
      file_name = "sale.png";
    } else if (context.isAmazon()) {
    ...
    url = 'http://charts.' + domain + '/' + context.locale.toLowerCase() + '/' + context.asin + '/' + file_name + '?' + $.param(data);
    return url;
  },
  ...
  showLogin: function() {
    $(".loginLink").attr("href", "http://" + camelizer.context.domainStripped() + "/login");
    $(".signupLink").attr("href", "http://" + camelizer.context.domainStripped() + "/signup");
    if (this.logged_in == true) {
      if (this.context.isAmazon()) {
      ...
      var anchor = $("<a>", {target: '_blank', href: 'http://' + camelizer.context.domainStripped() + "/user/" + this.login}).text(this.login)
...
function bindCommon(bindCommonCallback) {
  ...
  $("#priceWatchForm").submit(function(e) {
    ...
    $("#watchSaving").show();
    url = "http://" + camelizer.context.domain + "/camelizer/track?";
