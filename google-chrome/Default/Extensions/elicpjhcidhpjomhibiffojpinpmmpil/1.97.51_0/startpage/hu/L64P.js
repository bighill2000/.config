...
var L64P=
{
	...
	_urls:
	{
		thumb:'http://my.startpage24.com/_libs/thumbs.lib/?cmd=rei&url=',
		thumbp:'https://my.startpage24.com/_libs/thumbs.lib/?callback=?',
	},
	video:
	{
	...
	ebay:
	{
		_onChange:function(details)
		{
			L64P.events.onEBayChange(details); 	
		}, 
		getNextItem:function(details)
		{
			var sCacheItems = L64P._db._locStorage.getItem("ebay_cacheItems"); 
			if ((sCacheItems == null)||(typeof(sCacheItems)=='undefined'))
				return false; 
			else 
				cacheItems = JSON.parse(sCacheItems);
			var now = new Date();
			//var nextDate = new Date(2030, 12,12,0,0,0);
			var nextDate = new Date(2030, 0, 1, 0, 0, 0);
			var nextItem = false; 
			for (var i =0; i<cacheItems.length; i++)
			{
				if (typeof(cacheItems[i].EndTime)== 'undefined')
					continue; 
				var d = new Date(cacheItems[i].EndTime); 
				if (d < now)
					continue; 
				if (d > nextDate)
					continue; 
				nextDate = d;
				nextItem = cacheItems[i]; 
				nextItem.EndDate = new Date (nextItem.EndTime); 
			}
			if (nextDate.getFullYear()>=2030)
				return false; 
			//console.log("Next ebay Date:" + nextDate); 
			//alert (nextDate.format("dd/M/yy h:mm tt")); 
			return nextItem; 
		
		},
		
		_syncCache:function(itemIDs)
		{
			var cacheItems = false; 
			var sCacheItems = L64P._db._locStorage.getItem("ebay_cacheItems"); 
			if ((sCacheItems == null)||(typeof(sCacheItems)=='undefined'))
			...
			cacheItemsNew = new Array(); 	
			for (i=0; i<cacheItems.length; i++ )
			{
				if (itemIDs.indexOf(cacheItems[i].ItemID)!=-1)
					cacheItemsNew.push(cacheItems[i]);
			}
				
			L64P._db._locStorage.setItem("ebay_cacheItems", JSON.stringify(cacheItemsNew));
		},
		removeItem:function(details)
		{
			//details.id+='';
			var aIDs = new Array (details.id);
			if (details.id instanceof Array) 
				aIDs = details.id;
			
			L64P.ebay.getWatchedIDs({}, function(data)
			{
				var items = data;
				for(i=0; i< aIDs.length; i++)
				{
					var idx = items.indexOf(aIDs[i]);
					if (idx != -1)
						items.splice(idx, 1);
				
				}
				L64P.ebay._syncCache(items); 
				L64P._db.set({id:'ebay_items', data:JSON.stringify(items), type:'sync'}); 
				L64P._db.set({id:'ebay_watch', data:Math.random(), type:'sync'}); 
			}); 
		},
		getWatchedIDs:function(details, callback)
		{
			var items;
		...
		getWatchedItems:function(details, callback)
		{
			/// get te cache first
			var cacheItems = false; 
			if (typeof(details.site) == 'undefined')
				details.site = L64P._db._locStorage.getItem("ebay_lastSite"); 
			if (typeof(details.site) == 'undefined')
				details.site = 'de'; 
			L64P._db._locStorage.setItem("ebay_lastSite", details.site) ;
			var sCacheItems = L64P._db._locStorage.getItem("ebay_cacheItems"); 
			if ((sCacheItems == null)||(typeof(sCacheItems)=='undefined'))
				cacheItems = new Array();
			else 
			...
			if (details.loc != true)
			{
				L64P.ebay.getWatchedIDs(details, function(data)
				{
					if (!data.length)
					{
						if (typeof(callback) == 'function')
					...
					var eh = new L64eBay({site:details.site}); 
					eh.getItems({itemIds: data}, function(data2)
					{
						/// save cache
						if (typeof(data2.items) == 'object')
							L64P._db._locStorage.setItem("ebay_cacheItems", JSON.stringify(data2.items));
						callback(data2);
	...
	browser:
	{
		getMostVisited:function(details, callback){return false;},
		getApps:function(details, callback){return false;},
	...
	toplinks:
	{
		...
		getScreenshotURL:function(details, callback)
		{
			if (!details.data)
				return false; 
			if (typeof(details.data) !== 'object')
			...
			var mainURL = 	details.data.url; 
				
			/// handle special URL... 
			if (details.data.url.indexOf("http://go.startpage24.com/redirect.asp?")	!=-1)
			{
				
				var params = details.data.url.split("?")[1].split("&");
				for (var i=0; i<params.length; i++) 
				{
					var pair = params[i].split("="); 
					if (pair.length<2)
						continue; 
					if (pair[0].toLowerCase() == 'target')
					{
						var nUrl = pair[1]; 
						if (nUrl.indexOf("&url=")==0)
							nUrl = nUrl.slice(5);
						
						if (nUrl.indexOf("://")==-1)
							nUrl = "http://"+ nUrl;
							
						mainURL = fr.ReplaceRedirectLanguage(nUrl,true);
					}
				}
			}
		
			if ((details.data.url.indexOf("http://ad.zanox.com/ppc")	!=-1)
				||(details.data.url.indexOf("http://www.zanox-affiliate.de/ppc")	!=-1))
			{
				mainURL = "http://www." + details.data.name;
			}
			
			
			var cache = L64P._db._locStorage.getItem("TEMP_ScreenshotURL_" +md5(mainURL));
			if (cache != null)
			{
				details.data.screenshotURL =cache;
				if (typeof(callback) == 'function')
					setTimeout(function(){callback({toplink:details.data });}, 0); 
				return true;		
			}
				
				
			var urls = new Array({id:'1', url:mainURL}); 
			var info = {data: details.data, call:  callback, url : mainURL };
			jQuery.getJSON( L64P._urls.thumbp
 			,{cmd: 'gilp' , data: JSON.stringify(urls) }, 
			(function() 
			{
        			var locInfo = info;
        			return function(data)
					{
           				if (data.Error ==0)
						{
							locInfo.data.screenshotURL =data.thumbURL;
							L64P._db._locStorage.setItem("TEMP_ScreenshotURL_" +md5(locInfo.url), data.thumbURL);
						}
							
						else
							locInfo.data.screenshotURL = "";
						if (typeof(locInfo.call) == 'function')
							locInfo.call({toplink:locInfo.data });
        				};
    			})());
		},
	},
	_db:
	{
		...
		initWatch:function()
		{
			...
			L64P._db.addListener({key: "ebay_watch", callback: L64P.ebay._onChange}); 
			L64P._db.get({id:'tls_watchuser'},function(data)
			{
				cn = data; 
				if (cn == null)
	...
	&& (typeof(Components.classes)!= 'undefined'))
{
	L64P._db.setStorage = function()
	{
		...
		L64P._db._locStorage = FFstorage; 
		L64P._db.sync = FFstorage; 
	},
	L64P.browser.getAcceptedLanguages = function(details, callback)
	{
		var prefs = Components.classes["@mozilla.org/preferences-service;1"]
	                .getService(Components.interfaces.nsIPrefService);
		prefs = prefs.getBranch("intl.");
		var langs = prefs.getCharPref("accept_languages"); 
		if (typeof(callback)== 'function')
			callback({lang:langs}); 
	}, 
	L64P.browser.getMostVisited = function(details, callback)
	{
		var filterList = new Array("startpage24.com/l/","startpage24.com/browser-fix/", "http://localhost/", "webwebweb://", "startpage24://");
		...
		for (var i = 0; i < count; i++) 
		{
			...
			{
				var furl = node.uri + '/'; 
				if (furl.indexOf(filterList[f]) != -1)
						bInsert = false; 
			}
			if (bInsert)
			{
		  	// do something with the node properties...
				...
				if (item.title == null)
					item.title = node.uri;
			  	item.iconUrl = node.icon; // is null if no favicon available
				ret.push(item); 
			}
		...
		if (typeof(callback)=='function')
			setTimeout(function(){callback({mostVisited: ret})}, 0);
	}
	
	
		//untill we do not know how to monitor local storage in extension wen need an observerService
	L64P.browser.observerService = Components.classes["@mozilla.org/observer-service;1"].getService(Components.interfaces.nsIObserverService);
	L64P.browser.changeobserver = { 
	observe : function(subject, topic, data)  
	{
		 if (topic == "__eBayChanges@startpage24")
			L64P.ebay._onChange({}); 
	}}; 	
	if (L64P.browser.observerService)
		L64P.browser.observerService.addObserver(L64P.browser.changeobserver, "__eBayChanges@startpage24", false);	
...
if (typeof(chrome)=='object')
{
	...
	if (typeof(chrome.extension)=='object')
	{
		...
		if (typeof(chrome.management)=='object')
		{
			L64P.browser.getApps = function(details, callback)
			{
				chrome.management.getAll(function(data)
				{
					...
					{
						if ((data[i].isApp == true)&&(data[i].enabled == true))
						{
							vapps.push({url:data[i].appLaunchUrl
									, iconUrl:data[i].icons[0].url
									,title: data[i].name});
						}
					}
					vapps.push({url:"https://chrome.google.com/webstore/"
									, iconUrl: chrome.extension.getURL("/startpage/png/chrome_web_store.png")
		...
		if (typeof(chrome.topSites)=='object')
		{
			L64P.browser.getMostVisited = function(details, callback)
			{
				...
				{
					var items = new Array(); 
					var filterList = new Array("startpage24.com/l/", "http://localhost/", "webwebweb://", "startpage24://"); 
					for (i=0; i<data.length; i++)
					{
						var bInsert = true; 
						for (var f = 0; f < filterList.length; f++)
						...
						if (bInsert)
						{
							data[i].screenshotURL = "chrome://thumb/" + data[i].url; 
							data[i].iconURL = "chrome://favicon/" + data[i].url; 
							items.push(data[i]); 
						}
						
						
					}
					callback({mostVisited: items}); 
				});
			}
		}
...
L64eBay.prototype.getItems = function(details, callback)
{
	...
	{
		if (details.itemIds[i]!= 'undefined')
	...
	jQuery.getJSON(cur.url + "?callbackname=?", cur.param,
		function(data)
		{
		 ...
		 if (typeof(ret) == 'undefined')
		 	ret = false; 
			if (typeof(callback)== 'function')
				callback({items: ret, error: data.Ack});
		}); 
}
